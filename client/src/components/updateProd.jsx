...
const UpdateProd = ({ setEditProductDialog, productDialogFooter, product }) => {
  console.log(product);

  const token = useSelector(store => store.tokenSlice.token)

  const [name, setName] = useState(product.name)
  const [descraption, setDescraption] = useState(product.descraption)
  ...
  const load = async (e) => {
    e.preventDefault()
    const updatedProd = { id: product._id, name, descraption: descraption, category, tags, quentity, price, image };
    setLoading(true);
    try {
